...
def extract_ips(raw_text: str):
    ips = set(IP_RE.findall(raw_text))
    clean_ips = set()
...
def analyze(raw_bytes: bytes, vt_api_key: str | None = None, dns_timeout: float = 5.0):
    ...
    if vt_api_key:
        ...
        intel['vt_ips'] = {ip: vt_ip(ip, vt_api_key) for ip in ips}
    result = {
...
def main():
    ...
    try:
        ...
        if res.get("intel"):
            ...
            # IPs VT table with percentages
            ip_map = intel.get("vt_ips") or {}
            if ip_map:
                sub("IPs (VirusTotal)")
                headers = ["IP", "VT", "harmless", "suspicious", "malicious", "undetected", "total"]
                widths = [40, 7, 10, 11, 10, 12, 6]
                rows = []
                for ip, obj in ip_map.items():
                    st = vt_parse_stats(obj)
                    if not st:
                        rows.append([ip, "[N/A]", "-", "-", "-", "-", "-"])
                        continue
